...
var LdmlGenerator = /** @class */ (function () {
    ...
    LdmlGenerator.prototype.countStoreElements = function (value) {
        ...
        return __spreadArray([], value, true).length;
    };
    /**
    ...
    LdmlGenerator.prototype.generateKeysSection = function () {
        ...
        for (var _i = 0, _a = this.keys; _i < _a.length; _i++) {
            var _b = _a[_i], keyId = _b[0], key = _b[1];
            // Skip if it's a standard key we already added
            if (keyId === 'K_SPACE' || keyId === 'K_BKSP')
                continue;
            for (var _c = 0, _d = key.outputs; _c < _d.length; _c++) {
                var _e = _d[_c], modifiers = _e[0], output = _e[1];
                var variantId = this.getVariantKeyId(keyId, modifiers);
                xml += "".concat(this.indent).concat(this.indent, "<key id=\"").concat(variantId, "\"");
                if (output) {
                    xml += " output=\"".concat(this.escapeXml(output), "\"");
                }
                xml += "/>\n";
            }
        }
    ...
    LdmlGenerator.prototype.generateHardwareLayers = function () {
        ...
        var sortedModifiers = __spreadArray([], modifierSets, true).sort(function (a, b) {
            if (a === 'none')
                return -1;
            if (b === 'none')
                return 1;
    ...
    LdmlGenerator.prototype.generateVariablesSection = function () {
        ...
        for (var _i = 0, userStores_1 = userStores; _i < userStores_1.length; _i++) {
            ...
            if (value.includes('..')) {
                ...
                var chars = __spreadArray([], value, true);
    ...
    LdmlGenerator.prototype.generateTransformsSection = function () {
        ...
        for (var _h = 0, setMappingGroups_1 = setMappingGroups; _h < setMappingGroups_1.length; _h++) {
            var _j = setMappingGroups_1[_h], patternKey = _j[0], group = _j[1];
            if (group.canUseSetMapping && group.rules.length > 0) {
                // Generate single set-mapped transform
                var analyzed = group.rules[0];
                var from = this.generateSetMappingPattern(analyzed.rule, analyzed.setMappings);
                var to = this.generateSetMappingOutput(analyzed.rule, analyzed.setMappings);
                if (from && to) {
                    // Check for platform/layer conditions in any of the grouped rules
                    var conditions = this.extractConditions(group.rules.map(function (r) { return r.rule; }));
                    ...
                    xml += "".concat(this.indent).concat(this.indent).concat(this.indent, "<!-- Set mapping: ").concat(group.rules.length, " rules condensed -->\n");
            ...
            for (var _b = 0, _c = group.rules; _b < _c.length; _b++) {
                ...
                if (hasIfSet) {
                    ...
                    this.skippedRules.push({
                        ...
                        options: __spreadArray([], this.optionNames, true),
