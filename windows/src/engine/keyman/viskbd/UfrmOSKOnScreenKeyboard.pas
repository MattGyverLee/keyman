...
type
  ...
  private
    ...
    procedure UpdateUserHeldModifiers(VKCode, ScanCode: DWORD; IsUp: Boolean);
...
const
  ...
  /// SCAN_FLAG_KEYMAN_KEY_EVENT (keyman32/keyman64.h): the scan code keyman32 stamps on its own
  /// injected key events, including the serializer's modifier restore presses.
  SCAN_KEYMAN_KEY_EVENT = $FF;
...
var
   ...
   * Scan code alone cannot do that job: TRIAGE.md records the measurement that `scan = 0` reliably
...
begin
  ...
  UpdateUserHeldModifiers(VKCode, scanCode, isUp);
...
procedure TfrmOSKOnScreenKeyboard.ShiftStateChange(kbdShift, asyncShift: TExtShiftState);
  ...
  // modifiers, so teardown released the user's own keys (I2177). See MODIFIER-PRODUCERS.md, "The
  // FCachedShiftState invariant". FUserHeldShiftState is a different field under a different rule
...
var
   ...
   * you reboot" class of report in TRIAGE.md. Releasing Right Ctrl cannot touch Left Ctrl, so it
 ...
 * was the OSK's alone -- MODIFIER-PRODUCERS.md said so in as many words. It is not proof. If the
 ...
 * Scan codes are still used for the two events that are nobody's hold: keyman32's own injections,
 * which carry SCAN_KEYMAN_KEY_EVENT, and Windows' AltGr-simulated left Control, which carries
 * SCAN_LEFT_CONTROL_SIMULATED and which this unit already discounts via IsSimulatedLControlDown.
 *
 ...
 *)
procedure TfrmOSKOnScreenKeyboard.UpdateUserHeldModifiers(VKCode, ScanCode: DWORD; IsUp: Boolean);
var
...
begin
  ...
  if ScanCode = SCAN_KEYMAN_KEY_EVENT then
    Exit;
