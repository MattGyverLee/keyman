...
|---|---|---|---|---|---|
| 1 | serializer batch restore — `keybd_shift_reset` (`keyman32/keybd_shift.cpp:177-193`), called from `PrepareInjectedInputBatch` (`:371`), emitted by `SendInput` (`serialkeyeventserver.cpp:390`) | modifier KEYDOWNs from the cache, no queued KEYUP (`keybd_shift.cpp:184`) | `ReconcileModifierCache` (`keybd_shift.cpp:250`) called at `:337`; `ComputeModifierReleaseState` (`:290`) called at `:343` | **mitigated, measured** | The reconcile runs before the restore loop, so a cache byte that was *already* stale when the batch began is cleared, and the restore half is handed `kbd`, never `releaseState`. Pinned by `PREPARE_INJECTED_INPUT_BATCH.*` in `keyman32/tests/keybd_shift.tests.cpp`, which go red if the reconcile call is deleted. **What is not mitigated is a byte this batch makes stale itself.** The `WM_KEYMAN_MODIFIER_EVENT` post at `k32_lowlevelkeyboardhook.cpp:196` runs before the "generated by Keyman" pass-through at `:224`, so the batch's own release KEYUP and restore KEYDOWN feed the cache. A balanced batch cancels out, but if the user releases the modifier while the batch is being built, the hook posts their KEYUP first and the restore press is applied last -- leaving the cache reporting a modifier nobody holds, with the OS latched to match. The reconcile cannot recover it, because cache and OS now *agree*. Characterised by `MODIFIER_CACHE_EVENT_ORDER.*`, and `GatingKeymanOwnEventsWouldLeaveTheCacheFollowingTheUser` shows what filtering by provenance would buy. The tag needed for that filter does not survive Right Shift -- see TRIAGE.md -- so `dwExtraInfo` is the channel used, via `EXTRAINFO_FLAG_KEYMAN_MODIFIER_WRAP`. **Verified end to end on a live engine:** `host32.exe --probe 1x2x3x --iterations 5` wedged Shift on 5 of 5 iterations against the shipped build and 0 of 5 against the fixed build, with the freeze confirmed active and rules confirmed firing in every iteration of both runs. Reports in `evidence/` |
