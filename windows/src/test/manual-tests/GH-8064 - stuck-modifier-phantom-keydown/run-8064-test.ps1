...
try {
  for ($iter = 1; $iter -le $Iterations; $iter++) {
    Write-Host ''
    Write-Host "--- iteration $iter of $Iterations ---"
    ...
    if (-not $Control) {
      ...
      Write-Host ('    freeze active: {0}' -f $(if ($frozeOk) { 'yes' } else { 'NO - controller still responding' }))
    }

    Start-Sleep -Milliseconds $ReleaseDelayMs
    Send-Key -Vk $mod.vk -Scan $mod.scan -Up $true -Extended $mod.ext
    Write-Host ('    modifier released{0}' -f $(if ($Control) { '' } else { ' (inside the stall)' }))
    ...
    if ($editWnd -ne [IntPtr]::Zero) {
      ...
      $typed = $delta
      Write-Host ('    typed "{0}" -> got "{1}"  transformed: {2}' -f $PROBE_TEXT, $delta, $transformed)
    } else {
      $transformed = $null
    }

    $held = Get-HeldModifiers
    if ($held.Count -gt 0) { Write-Host ('    [FAIL] stuck: {0}' -f ($held -join ', ')) -ForegroundColor Red }
    else { Write-Host '    [ok] no modifier held' }
